...
// Default status hamesha pending rahega agar URL mein nahi hai
$currentFilter = $request->getGet('status') ?? 'pending';

// 🔥 Dynamic Label Logic
$pageType = $type ?? 'post'; // Controller se aane wala type
...
<style>
    ...
    .stat-card-pro:hover { transform: translateY(-3px); box-shadow: 0 8px 15px rgba(0,0,0,0.05); }
    .stat-card-pro.active { border-bottom: 3px solid var(--primary-blue) !important; background: #fcfdfe; }
    ...
    .user-name-pro { color: #000 !important; font-weight: 700; font-size: 14px; }
                            ...
                            <?php if(!empty($reports)): ?>
                                ...
                                <?php 
                                    ...
                                    if ($item->status == 'pending' && !empty($item->locked_by) && $item->locked_by != $currentAdminId) {
                                        // Agar lock 15 min purana nahi hai
                                ...
                                <tr style="border-bottom: 1px solid var(--border-soft);">
                                    <td class="align-middle px-4">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <div class="user-name-pro"><?= esc($item->reporter_name ?? 'User') ?></div>
                                    ...
                                    <td class="align-middle text-right px-4">
                                        <div class="btn-group">
                                            ...
                                            <?php if($isLocked): ?>
                                                <button type="button" class="action-btn-pro btn-locked" title="Admin ID #<?= $item->locked_by ?> is reviewing this.">
    ...
    document.addEventListener('click', function (e) {
        ...
        if (btn) {
            ...
            Swal.fire({
                ...
                text: "This will remove the report log permanently.",
                icon: 'warning',
