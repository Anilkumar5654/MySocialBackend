...
<style>
    ...
    .channel-details-pro { text-align: center; padding: 20px; }
    
    .owner-avatar-pro { width: 60px; height: 60px; border-radius: 12px; border: 1px solid var(--border-soft); object-fit: cover; }
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center">
            <h1 style="color: var(--text-dark); font-weight: 700; letter-spacing: -0.5px;">
                <i class="fas fa-tv mr-2 text-primary"></i> Review Channel Report
            </h1>
            <a href="<?= base_url('admin/reports/channels') ?>" class="btn btn-light btn-sm shadow-sm" style="font-weight: 600; border: 1px solid var(--border-soft); border-radius: 8px;">
            ...
            <div class="col-lg-8">
                <div class="card card-pro">
                    <div class="card-header-pro d-flex justify-content-between align-items-center">
                        <h6 class="text-dark m-0 font-weight-bold"><i class="fas fa-info-circle mr-2 text-primary"></i> CHANNEL INFO</h6>
                    ...
                    <div class="card-body p-0">
                        <?php if($content): ?>
                            ...
                            <img src="<?= get_media_url($content->cover ?? '', 'post_image') ?>" class="channel-cover-pro">
                            
                            <div class="channel-details-pro">
                                <img src="<?= get_media_url($content->avatar ?? '', 'profile') ?>" class="channel-avatar-pro mb-3">
                                
                                <h2 class="black-text mb-0"><?= esc($content->name ?? 'Unknown Channel') ?></h2>
                                <p class="text-primary font-weight-bold mb-3">@<?= strtoupper(esc($content->handle ?? $content->slug ?? '')) ?></p>

                                <div class="row justify-content-center mb-4">
                                    <div class="col-auto">
                                        <span class="badge shadow-sm p-2" style="background: #fff; color: #000; border: 1px solid var(--border-soft);">
                                            <i class="fas fa-users mr-1 text-primary"></i> <?= number_format($content->subscribers_count ?? 0) ?> Subscribers
                                        </span>
                                    </div>
                                    <div class="col-auto">
                                        <span class="badge shadow-sm p-2" style="background: #fff; color: #000; border: 1px solid var(--border-soft);">
                                            <i class="fas fa-video mr-1 text-info"></i> <?= number_format($content->videos_count ?? 0) ?> Videos
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="row justify-content-center">
                                    <div class="col-md-10">
                                        <div class="info-box-pro">
                                            <div class="label-pro">Channel Description</div>
                                            <div class="text-dark font-weight-500"><?= esc($content->description ?? 'No description provided for this channel.') ?></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <?php else: ?>
                            <div class="text-center py-5">
                                ...
                                <h5 class="text-muted font-weight-bold">Channel has been deleted</h5>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
                        ...
                        <div class="d-flex align-items-start">
                            ...
                            <div style="flex: 1;">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="label-pro">Date Reported</div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card card-pro">
                    <div class="card-header-pro text-center">
                        <h6 class="text-dark m-0 font-weight-bold">OWNER PROFILE</h6>
                    </div>
                    <div class="card-body text-center py-4">
                        <?php if($accusedUser): ?>
                            <img src="<?= get_media_url($accusedUser->avatar, 'profile') ?>" class="owner-avatar-pro">
                            <h5 class="black-text mt-2 mb-0"><?= esc($accusedUser->name) ?></h5>
                            <p class="text-primary font-weight-bold small mb-2">@<?= strtoupper(esc($accusedUser->username)) ?></p>
                            <div class="mt-2">
                                <?php if(($accusedUser->is_banned ?? 0) == 1): ?>
                                    <span class="badge" style="background: rgba(253, 57, 122, 0.1); color: var(--accent-red); border: 1px solid var(--accent-red); padding: 5px 12px;">BANNED</span>
                                <?php else: ?>
                                    <span class="badge" style="background: rgba(10, 187, 135, 0.1); color: var(--accent-green); border: 1px solid var(--accent-green); padding: 5px 12px;">ACTIVE</span>
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                            <p class="text-muted font-weight-bold">Owner not found</p>
                        <?php endif; ?>
                    </div>
                </div>
                            ...
                            <form action="<?= base_url('admin/reports/action') ?>" method="post" id="actionForm">
                                ...
                                <?php if(has_permission('users.ban')): ?>
                                    <button type="button" onclick="confirmAction('ban_user')" class="btn-action-pro" style="background: var(--accent-red); color: #fff;">
                                        <i class="fas fa-ban mr-2"></i> BAN OWNER ACCOUNT
                                    </button>
                                <?php endif; ?>
                            </form>
...
document.addEventListener("DOMContentLoaded", function() {
    window.confirmAction = function(actionType) {
        ...
        if(actionType === 'dismiss') { title='Dismiss Report?'; text='Confirm if no violation was found.'; color='#abb3ba'; btnText='Yes, Dismiss'; }
        else if(actionType === 'delete_content') { title='Delete Channel?'; text='This will remove the channel and all its data!'; color='#ff9f43'; btnText='Yes, Delete Channel'; }
        else if(actionType === 'ban_user') { title='Ban Owner?'; text='Suspend owner account for policy violation?'; color='#fd397a'; btnText='Yes, Ban Account'; }

        Swal.fire({
            title: title, text: text, icon: 'warning',
            input: 'text', inputPlaceholder: 'Enter a reason for this action...',
