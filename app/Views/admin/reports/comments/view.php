    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center">
            <h1 style="color: var(--text-dark); font-weight: 700; letter-spacing: -0.5px;">
                <i class="fas fa-comment-slash mr-2 text-primary"></i> Review Comment
            </h1>
            <a href="<?= base_url('admin/reports/comments') ?>" class="btn btn-light btn-sm shadow-sm" style="font-weight: 600; border: 1px solid var(--border-soft); border-radius: 8px;">
            ...
            <div class="col-lg-8">
                <div class="card card-pro">
                    <div class="card-header-pro d-flex justify-content-between align-items-center">
                        <h6 class="text-dark m-0 font-weight-bold"><i class="fas fa-quote-left mr-2 text-primary"></i> REPORTED COMMENT</h6>
                    ...
                    <div class="card-body p-4">
                        <?php if($content): ?>
                            ...
                            <div class="comment-box-pro">
                                <?= esc($content->comment ?? $content->body ?? 'No text available') ?>
                            </div>

                            <p class="text-muted small mb-0">
                                <i class="far fa-calendar-alt mr-1"></i> Posted on: <?= date('d M Y, h:i A', strtotime($content->created_at)) ?>
                            </p>

                        <?php else: ?>
                            <div class="text-center py-5">
                                <i class="fas fa-trash-alt fa-3x text-danger mb-3 opacity-25"></i>
                                <h5 class="text-muted font-weight-bold">Comment has been deleted</h5>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                <div class="card card-pro">
                    <div class="card-header-pro text-center">
                        <h6 class="text-dark m-0 font-weight-bold">COMMENT AUTHOR</h6>
                    </div>
                    <div class="card-body text-center py-4">
                        ...
                        <?php else: ?>
                            <p class="text-muted font-weight-bold">Author not found</p>
                        <?php endif; ?>
                    </div>
                </div>
                            ...
                            <form action="<?= base_url('admin/reports/action') ?>" method="post" id="actionForm">
                                ...
                                <?php if(has_permission('users.ban')): ?>
                                    <button type="button" onclick="confirmAction('ban_user')" class="btn-action-pro" style="background: var(--accent-red); color: #fff;">
                                        <i class="fas fa-ban mr-2"></i> BAN AUTHOR
                                    </button>
                                <?php endif; ?>
                            </form>
...
document.addEventListener("DOMContentLoaded", function() {
    window.confirmAction = function(actionType) {
        ...
        else if(actionType === 'delete_content') { title='Delete Comment?'; text='This will permanently remove the comment.'; color='#ff9f43'; btnText='Yes, Delete'; }
        else if(actionType === 'ban_user') { title='Ban Author Account?'; text='This will block the user from the platform.'; color='#fd397a'; btnText='Yes, Ban User'; }
